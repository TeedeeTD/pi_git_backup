...
class Config:
    ...
    DEFAULTS = {
        ...
        "tracking": {
            ...
            "marker_size": 0.1     # [Mét] Kích thước thực tế của in mã ArUco (tính cạnh đen ngoài cùng).
...
class FFmpegStreamer:
    ...
    Sử dụng Subprocess để gọi lệnh FFmpeg hệ thống.
    Sử dụng Queue để đệm frame giữa Python và FFmpeg.
    """
    def __init__(self, push_url, width, height, fps):
        ...
        self.cmd = [
            ...
            '-vcodec', 'rawvideo', 
            '-pix_fmt', 'bgr24',    # Định dạng: BGR (OpenCV Default)
            '-s', f'{width}x{height}', 
            '-r', str(fps), 
            '-i', '-',              # Input: STDIN Pipe
    ...
    def _worker(self):
        """Thread worker: Lấy frame từ Queue và nhét vào họng (STDIN) của FFmpeg."""
        while self.running:
            try:
    ...
    def write(self, frame):
        ...
        Logic: Resize -> Put vào Queue. Nếu Queue đầy -> Drop frame.
        """
        if not self.running: return
        
        # Resize ảnh về đúng kích thước đã cấu hình cho FFmpeg
        ...
        try:
            self.queue.put_nowait(frame_out) # put_nowait: Không chờ, nếu đầy thì văng Exception Full
        except queue.Full:
...
class SiyiGimbalController:
    ...
    # --- [NEW] HÀM MỚI THÊM ĐỂ SET GÓC TUYỆT ĐỐI ---
...
def main():
    ...
    logging.info(">>> SYSTEM STARTING (FIXED -90 DEGREE MODE)...")
    ...
    # --- [MODIFIED] SET GÓC -90 ĐỘ BAN ĐẦU ---
    # Thay vì center, ta gọi set_angle(-90 độ pitch) để cam nhìn thẳng xuống đất.
    # Gửi 3 lần để đảm bảo lệnh được nhận (UDP có thể rớt gói)
    ...
    try:
        while True:
            ...
            # ---------------------------
            # BƯỚC 3: TÍNH TOÁN POSE (ĐÃ SỬA ĐỔI HỆ TRỤC CHO GÓC -90)
            # ---------------------------
            # Khởi tạo các biến tọa độ theo hệ Body Frame
            ...
            if last_ids is not None and last_corners is not None:
                ...
                try:
                    ...
                    # --- [LOGIC MỚI] CHUYỂN ĐỔI HỆ TỌA ĐỘ ---
                    # Khi cam nhìn xuống:
                    # - tvec[0] (Cam X) = Drone Trái/Phải (Body Y)
                    # - tvec[1] (Cam Y) = Drone Lùi/Tiến (Ngược Body X)
                    # - tvec[2] (Cam Z) = Độ cao (Altitude)
                    ...
                    # --- [MODIFIED] HIỂN THỊ THÔNG TIN BODY FRAME ---
            ...
            # ---------------------------
            # BƯỚC 4: ĐIỀU KHIỂN GIMBAL (VÔ HIỆU HÓA PID)
            # ---------------------------
            # Vì ta muốn giữ Cam cố định -90 độ, nên ta KHÔNG gọi gimbal.rotate() để tracking nữa.
            # Logic PID cũ đã được bỏ qua để tránh cam tự xoay đi chỗ khác.
            pass
            ...
            # Sleep để giữ FPS ổn định, tránh chiếm dụng 100% CPU
            if wait_time > 0:
            ...
            if frame_count >= 10:
                ...
                # --- [MODIFIED] LOG DỮ LIỆU ĐÃ CHUYỂN ĐỔI ---
                log_payload = [
                    datetime.now().strftime("%H:%M:%S.%f")[:-3],
                    f"{algo_time_ms:.2f}",
                    f"{loop_ms:.2f}",
                    f"{fps:.2f}",
                    f"{body_x:.3f}",   # Body Forward/Backward
                    f"{body_y:.3f}",   # Body Left/Right
                    f"{body_z:.3f}",   # Altitude
                    f"{marker_yaw:.2f}" # Heading Offset
                ]
