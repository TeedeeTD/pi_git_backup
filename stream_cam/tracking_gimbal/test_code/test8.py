...
class CSVDataLogger(threading.Thread):
    ...
    def __init__(self, prefix="track_log"):
        ...
        # Header file CSV
        self.header = ["Timestamp", "Algo_ms", "Loop_ms", "FPS", "Pos_X", "Pos_Y", "Pos_Z", "Roll", "Pitch", "Yaw"]
        self.running = True
...
def main():
    ...
    logging.info(">>> SYSTEM STARTING (CLEAN VERSION)...")
    ...
    # --- SỬA Ở ĐÂY ---
    # Thay vì center, ta gọi set_angle(-90 độ pitch)
        ...
        while True:
            ...
            if last_ids is not None and last_corners is not None:
                ...
                try:
                    ...
                    # --- PID LOGIC ---
                    c = last_corners[0][0]
                    curr_cx = int((c[0][0] + c[2][0]) / 2) # Tâm Marker X
                    curr_cy = int((c[0][1] + c[2][1]) / 2) # Tâm Marker Y
                    
                    error_x = curr_cx - center_x
                    error_y = curr_cy - center_y
                    
                    # Tính Yaw Output
                    if abs(error_x) > trk_conf['deadzone']:
                        yaw_output = int(error_x * trk_conf['kp_yaw'])
                    else:
                        yaw_output = 0
                        
                    # Tính Pitch Output (đảo dấu do trục ảnh Y ngược trục Pitch)
                    if abs(error_y) > trk_conf['deadzone']:
                        pitch_output = int(-error_y * trk_conf['kp_pitch'])
                    else:
                        pitch_output = 0
            ...
            # ---------------------------
            # BƯỚC 4: ĐIỀU KHIỂN GIMBAL
            # ---------------------------
            if trk_conf['active']:
                gimbal.rotate(yaw_output, pitch_output)
            ...
            if frame_count >= 10:
                ...
                # Đóng gói dữ liệu và gửi sang Thread Log
                log_payload = [
