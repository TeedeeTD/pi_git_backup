...
class Config:
    DEFAULTS = {
        "system": {
            "detect_scale": 0.7,
            "skip_frame": 0,       # Đặt 0 để đo chính xác FPS thực tế của thuật toán
            "input_width": 1280,
            "input_height": 720,
            "stream_scale": 0.5,
            "target_fps": 30,
        ...
        "camera_matrix": {
            # GIỮ NGUYÊN THAM SỐ CAMERA NHƯ YÊU CẦU
...
class CSVDataLogger(threading.Thread):
    def __init__(self, prefix="benchmark_log"):
        ...
        # [MODIFIED] Header tập trung vào các chỉ số Benchmark và Raw Camera Data
        self.header = [
            "Timestamp", 
            "Frame_ID",
            "B3_Infer_ms",    # Thời gian chỉ riêng bước detectMarkers
            "C3_Algo_ms",     # Thời gian từ lúc nhận ảnh RAM -> Ra tọa độ (Latency thuật toán)
            "Loop_ms",        # Tổng thời gian vòng lặp (dùng tính B2)
            "FPS_Inst",       # FPS tức thời
            "D1_Detect",      # 1: Có marker, 0: Không
            "Cam_X",          # Tọa độ X trong Camera Frame
            "Cam_Y",          # Tọa độ Y trong Camera Frame
            "Cam_Z",          # Tọa độ Z trong Camera Frame
            "Cam_Roll",       # Góc Roll
            "Cam_Pitch",      # Góc Pitch
            "Cam_Yaw"         # Góc Yaw
        ]
...
class FFmpegStreamer:
    def __init__(self, push_url, width, height, fps):
        ...
        # Cấu hình FFmpeg tối ưu latency, hạn chế log rác
        self.cmd = [
            'ffmpeg', '-y', '-f', 'rawvideo', '-vcodec', 'rawvideo', 
            '-pix_fmt', 'bgr24', '-s', f'{width}x{height}', '-r', str(fps), 
            '-i', '-', 
            '-c:v', 'libx264',          # Dùng libx264 cho tương thích cao (hoặc h264_v4l2m2m trên Pi)
            '-pix_fmt', 'yuv420p', 
            '-g', '30', '-preset', 'ultrafast', 
            '-tune', 'zerolatency', 
            '-b:v', '2000k',
            '-loglevel', 'error',       # Ẩn log không cần thiết
...
def main():
    ...
    logging.info(">>> STARTING BENCHMARK MODE (METRICS: B1, B2, B3, C3, D1)...")
    ...
    # --- BIẾN ĐO ĐẠC METRICS ---
    frame_count = 0
    total_frames_processed = 0      # Mẫu số cho D1
    total_detected_frames = 0       # Tử số cho D1
    all_loop_times = []             # Lưu lại để tính B2 (1% Low)
    ...
    try:
        while True:
            ...
            # [C3 START] Bắt đầu đo độ trễ thuật toán (Từ lúc có ảnh -> Kết quả)
            c3_start = time.perf_counter()

            # ---------------------------
            # BƯỚC 2: PHÁT HIỆN ARUCO (B3)
            # ---------------------------
            detect_counter += 1
            ...
            if should_detect:
                ...
                # [B3 START] Đo thời gian Inference
                ...
                # [B3 END] Kết thúc đo Inference
                b3_infer_ms = (time.perf_counter() - b3_start) * 1000
                
                # [D1 MEASUREMENT] Cập nhật biến đếm Detection Rate
                ...
                if ids is not None and len(ids) > 0:
                    ...
                    # ---------------------------
                    # BƯỚC 3: TÍNH TOÁN POSE (C3 tiếp tục)
                    # ---------------------------
                    ...
                    # [MODIFIED] Chỉ lấy Raw Coordinate và Euler Angles
                    cam_x = tvec[0][0]
                    cam_y = tvec[1][0]
                    cam_z = tvec[2][0]
            ...
            # [C3 END] Kết thúc đo Algo Latency
            c3_algo_ms = (time.perf_counter() - c3_start) * 1000

            # ---------------------------
            # BƯỚC 4: QUẢN LÝ GIMBAL
            # ---------------------------
            ...
            # ---------------------------
            # BƯỚC 6: TÍNH TOÁN & LOGGING
            # ---------------------------
            elapsed = time.perf_counter() - loop_start
            loop_ms = elapsed * 1000
            
            # Lưu loop_ms để tính B2
            ...
            if frame_count >= 1:
                ...
                # Log đầy đủ các chỉ số yêu cầu
                log_payload = [
                    datetime.now().strftime("%H:%M:%S.%f")[:-3],
                    detect_counter,
                    f"{b3_infer_ms:.2f}",   # B3
                    f"{c3_algo_ms:.2f}",    # C3
                    f"{loop_ms:.2f}",       # Loop time
                    f"{fps:.2f}",           # FPS Inst
                    is_detected,            # D1 raw
                    f"{cam_x:.3f}",         # Cam X
                    f"{cam_y:.3f}",         # Cam Y
        ...
        # ---------------------------
        # TỔNG HỢP KẾT QUẢ BENCHMARK (SUMMARY)
        # ---------------------------
        print("\n" + "="*40)
