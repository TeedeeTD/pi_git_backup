...
class CSVDataLogger(threading.Thread):
    ...
    def __init__(self, prefix="track_log"):
        ...
        # [MODIFIED] Thêm header cho các giá trị Cam raw
        self.header = [
...
def main():
    ...
    try:
        while True:
            ...
            # [MODIFIED] Khởi tạo biến cho Raw Camera Data
            ...
            if last_ids is not None and last_corners is not None:
                ...
                try:
                    ...
                    # [MODIFIED] Tính toán Euler Angles (Roll, Pitch, Yaw) từ Rotation Matrix
                    rmat, _ = cv2.Rodrigues(rvec)
                    
                    # Logic tính Euler Angles từ Rotation Matrix
                    ...
                    # Tính góc xoay Yaw của Marker (Heading) cho Body Frame
            ...
            if frame_count >= 10:
                ...
                # [MODIFIED] Log thêm dữ liệu Raw Cam
                log_payload = [
                    ...
                    f"{body_z:.3f}",   
                    f"{marker_yaw:.2f}",
                    f"{cam_x_raw:.3f}",   # Raw X
                    f"{cam_y_raw:.3f}",   # Raw Y
                    f"{cam_z_raw:.3f}",   # Raw Z
                    f"{cam_roll:.2f}",    # Roll
