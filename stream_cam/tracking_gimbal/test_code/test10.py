...
def get_body_position(tvec, g_roll, g_pitch, g_yaw):
    ...
    # Gimbal Frame chuẩn (khi góc=0): X-Trước, Y-Phải, Z-Xuống
    # OpenCV Frame: Z-Trước, X-Phải, Y-Xuống
    # => Mapping: Body_X = Cam_Z (xoay), Body_Y = Cam_X (xoay), Body_Z = Cam_Y (xoay)
    
    # Tuy nhiên, do tvec gốc là X-Phải, Y-Xuống, Z-Sâu. 
    # Sau khi nhân R_gimbal, vector P_body_raw vẫn giữ tính chất hướng đó nhưng đã bù góc.
    # Ta map lại thủ công cho dễ hiểu:
    
    # Body X (Tiến) tương ứng với trục Z của Camera (Depth)
    final_x = P_body_raw[2] 
    # Body Y (Phải) tương ứng với trục X của Camera
    final_y = P_body_raw[0]
    # Body Z (Cao/Xuống) tương ứng với trục Y của Camera
    final_z = P_body_raw[1]
...
class SiyiGimbalController:
    """
    Class điều khiển Gimbal SIYI - Đã nâng cấp để NHẬN dữ liệu góc quay.
    """
    def __init__(self, ip, port):
        self.ip = ip
        self.port = port
        self.sock = socket.socket(socket.AF_INET, socket.SOCK_DGRAM)
        # Set timeout để recv không bị treo mãi mãi
        self.sock.settimeout(0.1) 
        self.seq = 0
        self.last_sent_time = 0
        
        # Biến lưu trữ góc Gimbal hiện tại (Đơn vị: Độ)
        self.current_roll = 0.0
        self.current_pitch = 0.0
        self.current_yaw = 0.0
        self.running = True

        # Thread lắng nghe phản hồi từ Gimbal
    ...
    def _listen_worker(self):
        """Luồng chạy ngầm để đọc dữ liệu UDP trả về"""
        while self.running:
            try:
    ...
    def _parse_packet(self, data):
        """Giải mã gói tin SIYI"""
        if len(data) < 10: return
        # Header Check: 0x55 0x66
        if data[0] != 0x55 or data[1] != 0x66: return

        cmd_id = data[6]
        
        # 0x0D: ATTITUDE DATA (Góc quay)
        if cmd_id == 0x0D:
            # Payload bắt đầu từ byte thứ 8 (sau SEQ và CMD_ID)
            # Cấu trúc: Yaw(2) Pitch(2) Roll(2) -> int16 little-endian
            try:
                yaw_raw, pitch_raw, roll_raw = struct.unpack('<hhh', data[8:14])
                # Đơn vị: 0.1 độ => Chia 10
    ...
    def request_attitude(self):
        """Gửi lệnh hỏi góc quay (CMD_ID 0x0D với payload rỗng)"""
        self.seq += 1
        msg_head = b'\x55\x66\x01\x00\x00' + struct.pack('<H', self.seq) + b'\x0D'
        packet = self._append_crc16(msg_head)
        try:
            self.sock.sendto(packet, (self.ip, self.port))
        except: pass

    def get_attitude(self):
        """Trả về 3 góc hiện tại"""
...
def main():
    ...
    logging.info(">>> SYSTEM STARTING (TRACKING + ATTITUDE LOGGING)...")
    ...
    try:
        while True:
            ...
            # --- 1. Gửi lệnh yêu cầu Gimbal gửi về toạ độ (Poll) ---
            # Cần gọi liên tục hoặc định kỳ để cập nhật biến self.current_yaw/pitch
            ...
            # --- 4. Tính toán Pose & PID ---
            body_x, body_y, body_z = 0, 0, 0
            marker_yaw = 0
            
            # Lấy góc Gimbal hiện tại (từ thread lắng nghe)
            ...
            if last_ids is not None and last_corners is not None:
                try:
                    ...
                    # --- [NEW LOGIC] NHÂN MA TRẬN ĐỂ RA TOẠ ĐỘ CHÍNH XÁC ---
                    ...
                    # PID Tracking Logic (Vẫn dùng pixel error để điều khiển Gimbal cho mượt)
                    c = last_corners[0][0]
            ...
            if frame_count >= 10:
                ...
                # --- [UPDATED] GHI LOG BAO GỒM CẢ GÓC GIMBAL ---
                log_payload = [
