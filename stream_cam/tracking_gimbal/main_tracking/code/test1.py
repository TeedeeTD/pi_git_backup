...
class Config:
    DEFAULTS = {
        "system": {
            ...
            "target_fps": 30,
            "gps_port": 5555,
            "ffmpeg_codec": "libx264" # [NEW] Mặc định an toàn
        },
        "connection": {
        ...
        if os.path.exists(config_path):
            try:
                with open(config_path, 'r') as f:
                    ...
                    # Update recursive dict (đơn giản hóa bằng update cấp 1)
...
class FFmpegStreamer:
    def __init__(self, push_url, width, height, fps, codec="libx264"):
        ...
        logging.info(f"Initializing FFmpeg Streamer with Codec: {codec}")

        # Cấu hình FFmpeg động dựa trên codec
        self.cmd = [
            'ffmpeg', '-y', '-f', 'rawvideo', '-vcodec', 'rawvideo', 
            '-pix_fmt', 'bgr24', '-s', f'{width}x{height}', '-r', str(fps), 
            '-i', '-', 
            '-c:v', codec,              # [MODIFIED] Sử dụng biến codec
            '-pix_fmt', 'yuv420p', 
        ...
        # Nếu dùng libx264 thì thêm tune zerolatency, còn hardware encoder thường không cần hoặc tự xử lý
        if "libx264" in codec:
            # Chèn tune zerolatency vào trước -b:v
...
def main():
    ...
    # Lấy thông tin Codec từ config
    selected_codec = cfg.get('system', 'ffmpeg_codec')
    if not selected_codec:
        selected_codec = "libx264" # Fallback

    logging.info(f">>> STARTING BENCHMARK MODE (Codec: {selected_codec})...")
    ...
    # [MODIFIED] Khởi tạo Streamer với codec từ config
    pusher = FFmpegStreamer(conn_conf['rtsp_push_url'], stream_w, stream_h, sys_conf['target_fps'], codec=selected_codec)
    ...
    try:
        while True:
            ...
            # [C3 START] Bắt đầu đo độ trễ thuật toán
